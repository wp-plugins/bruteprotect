...
define( 'BRUTEPROTECT_VERSION', '2.0.2' );
...
class BruteProtect {
	...
	function brute_maybe_use_secure_login() {
		...
		if ( ! bruteprotect_has_secure_login() || ! bruteprotect_is_pro() ) {
	...
	function brute_call( $action = 'check_ip', $request = array(), $sign = false ) {
		...
		$request['host']                 = strval( $this->brute_get_local_host() );
		...
		$request['subdirectory']         = $this->is_subdirectory();
		$request['multisite']            = "0";
...
class bp_Widget extends WP_Widget {
	...
	public function form( $instance ) {
		...
		<p>
			...
			<label
				for="<?php echo $this->get_field_id( 'showsitestats' ); ?>"><?php _e( 'Show your stats?' ); ?></label></label>
