	...
	class BruteProtect_Shoutouts extends BruteProtect
	{
		...
		function generate_notice() {
			...
			if(is_array($shoutouts)) :  foreach($shoutouts as $slug => $so) :
				?>
					<div class="updated after-h2 bruteprotect-shoutout" style="padding: 20px; border-color: #ff6600; background-color: #fff; position: relative;">
						<img src="<?php echo BRUTEPROTECT_PLUGIN_URL ?>/images/BruteProtect-lock.png" style="float: left; margin-right: 20px;"/>
						<?php echo $so; ?>
						<div style="clear: both;"></div>
					    <a href="<?php echo add_query_arg( 'dismiss_bruteprotect_shoutout', $slug ); ?>" class="dismiss" style="position: absolute;top: 5px;right: 10px;font-weight: bold;color: #AAA;font-size: 20px;" onclick="jQuery('.bruteprotect-shoutout').slideUp();">&times;</a>
					</div>
				<?php 
				return;
			endforeach; endif;
